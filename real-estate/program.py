...
def query_data(data):
    ...
    prices = [p.price for p in data]  # projection or items  # the set to process
    ave_price = statistics.mean(prices)
    print("The average home price is ${:,}".format(int(ave_price)))

    two_bed_homes = [p for p in data if p.beds == 2]  # projection or items  # the set to process  # test / condition
    ...
    print("The average home price of a 2-bedroom home is ${:,}, baths={}, sq ft={:,}".format(int(ave_price), round(ave_baths, 1), round(ave_sqft, 1)))

    # ** Make tuple with condition
    two_bed_homes2 = (
        p for p in data if announce(p, "2-bedrooms, found {}".format(p.beds)) and p.beds == 2  # projection or items  # the set to process  # test / condition
    )
    homes = []
